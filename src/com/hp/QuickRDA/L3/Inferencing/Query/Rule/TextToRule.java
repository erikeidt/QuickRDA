...
import com.hp.QuickRDA.L3.Inferencing.Query.PatternDefinition.QueryVariableKindEnum;
...
public class TextToRule {
	...
	public TextToRule ( BufferedReader br, NamedRuleManager pm, ConceptManager cm ) {
		itsPM = pm;
		itsCM = cm;
		itsTkn = new ScanReader ( br, "Query Pattern Parser", "text parameter" );
	}

	public static void indent ( int t, PrintStream out ) {
		for ( int i = 0; i < t; i++ )
			out.print ( "\t" );
	}
	...
	public HLQueryStatement parseQueryStatement ( IList<NamedQueryVariable> nqv ) {
	...
	private NamedPatternManager.NamedPattern assemblePatternFrame ( String patternName, IList<HLQueryStatement> inferredStatements, IList<NamedQueryVariable> nqv, HLQueryStatement hlqs ) {
		...
		*/
		...
		hlqs.genStatements ( null, itsDefaultViz, false );
		...
		for ( int i = 0; i < varCnt; i++ ) {
			...
			*/
			vd [ i ] = new PatternDefinition.VariableDefinition ( v.itsKind, v.itsViz );
		}
		...
		*/
		...
		DMIElem [] values = new DMIElem [ nqv.size () ];
		...
		/*		NamedPatternManager.NamedPattern npf = new NamedPatternManager.NamedPattern ( patternName, bf );
				return npf;
		*/
		return new Rule ( patternName, itsCM.itsGraph, hlqs, inferredStatements, vd, values );
	}
